...
void write_bundles(const string &pathBundles, const std::string output_path, const std::string suffix, uint32_t *&nFibers, uint16_t **&nPoints, float ****&Points){
    ...
    for(unsigned int z = 0; z < bundlesDir.size(); z++){
    	...
    	string bundlesdata_path = output_path + bundlesDir[z] + "_" + suffix + ".bundlesdata";
	    ...
	    bundlesfile.open( output_path + bundlesDir[z] + "_" + suffix + ".bundles", ios::out);
...
int main(int argc, char const *argv[])
{
	...
	write_bundles(argv[1], argv[2], argv[4], nFibers, nPoints, Points);
